...
const readExistingApiKeys = (clientEnvPath: string): Record<string, string> => {
    ...
    try {
        ...
        console.warn(
            chalk.yellow(`Warning: Could not read existing .env file: ${err}`),
        );
    }
 ...
 */
const writeApiKeysToFile = async (
    filePath: string,
    newContent: string,
): Promise<void> => {
    try {
        ...
        const separator =
            filteredContent && !filteredContent.endsWith("\n") ? "\n" : "";
...
const promptForApiKeys = async (existingKeys: Record<string, string>) => {
    ...
    console.log(chalk.gray("Configure your API keys\n"));
    ...
    for (const [keyName, config] of Object.entries(API_KEYS)) {
        ...
        if (response.value !== undefined) {
            ...
            if (response.value) {
                console.log(
                    chalk.green(`✓ ${hasExisting ? "Updated" : "Set"} ${keyName} key`),
                );
            }
        } else {
