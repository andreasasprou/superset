...
```
superset-sh/superset:main (upstream)
              │
              │ sync periodically
              ▼
    andreasasprou/superset:main
              │
              │ contains: upstream + release-specific files
              │ (workflow, commands, electron-builder owner, auto-updater URL)
              │
              ▼
    andreasasprou/superset:release
              │
              │ reset to main, then merge PRs from upstream
              │
              ▼
         v1.0.x tags
```
...
**Branches:**
- `main` - Synced with upstream + contains release-specific config (version, owner, URLs)
- `release` - Reset to main when ready, then merge PRs → triggers auto-versioned release

**Key insight:** Release-specific files (workflow, electron-builder owner, auto-updater URL, version) live in `main`, not just `release`. This makes `release` a clean slate that can be reset anytime.
...
# 2. Merge to your main (keeps release-specific files)
git checkout main
git merge upstream/main
# Resolve conflicts: keep YOUR version, owner, URLs in release-specific files
git push origin main
```

## Releasing New Version

When ready to release with specific PRs:

```bash
# 1. Reset release to main (clean slate)
git checkout release
git fetch origin
git reset --hard origin/main

# 2. Merge PRs you want (from upstream)
git fetch upstream pull/XXX/head:pr-XXX
git merge pr-XXX --no-ff -m "Merge superset-sh/superset#XXX"

# 3. Push (triggers auto-release)
git push --force-with-lease origin release
```

**Note:** Use `--force-with-lease` since we're resetting the branch. This is intentional.
